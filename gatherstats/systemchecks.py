"""
The :py:mod:`gatherstats` application ships with some custom system
checks which ensure that required settings are present. These system checks are registered by
the :py:class:`~gatherstats.apps.Config` class's
:py:meth:`~gatherstats.apps.AssetsConfig.ready` method.
    ...
    for idx, name in enumerate(REQUIRED_SETTINGS):
        ...
        if value is None or value == '':
            errors.append(Error(
                ...
                id='gatherstats.E{:03d}'.format(idx + 1),
